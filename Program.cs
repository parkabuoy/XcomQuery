...
// build datatable out of csv file (directly swf's id reference sheets)
...
// write header row
Show($"id\tlName\tnName\trank\tstatus{perkNames}");
...
foreach (Soldier thisSoldier in roster.Where(x => x.status != "Dead" && !string.IsNullOrEmpty(x.lName)))
{
  ...
  {
    // add tab to perkFlags string for this position
    perkFlags += "\t";
  ...
  Show($"{thisSoldier.id}\t{thisSoldier.lName}\t{thisSoldier.nName}\t{thisSoldier.rank}\t{thisSoldier.status}{perkFlags}");
}
